...
bot_name = 'alphabot' + str(random.randint(0, 100))
...
def play_game():
    ...
    info = info_phase()
    logging.info('Info Phase: %s' % info)
    ...
    while running:
        ...
        verify, action_enemy = get_direction(count_turn, player_index)
        if verify == -1:
            break
        ...
        s = map_to_state(mapp, old_mapp, s, 0, tmp_head)  # TODO: Map to state
